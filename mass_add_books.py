...
error = False
if shelf_id.isdigit():
    r = requests.post(serveradress + '/login?next=/',
                      data={'username': username, 'password': password, 'submit': '', 'remember_me': 'on', 'next': '/'})
            ...
            for current_row in reader:
                if "id" in current_row:
                    if current_row['id'].isdigit():
                        add = requests.get(serveradress + '/shelf/add/' + shelf_id + '/' + current_row['id'],
